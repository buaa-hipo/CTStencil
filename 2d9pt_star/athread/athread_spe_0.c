        ...
        for (g = 0; g < ng; g++)
        {
            ...
            for (y = 0; y < DIMY_final; y++)
            {
                ...
                for (x = 0; x < DIMX_final; x++)
                {
                    ...
                    E1 = C + 1;
                    E2 = C + 2;
                    local_tOut[x] = c0 * local_tIn[C] + c1 * local_tIn[W1] + c2 * local_tIn[W2] + c3 * local_tIn[E1] + c4 * local_tIn[E2] + c5 * local_tIn[N1] + c6 * local_tIn[N2] + c7 * local_tIn[S1] + c8 * local_tIn[S2];
                }
                DMA_push = 0;
