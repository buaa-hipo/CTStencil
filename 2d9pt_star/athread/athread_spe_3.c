...
void spe_func(struct spe_parameter *spe_param)
{
    ...
    int aligned;
    DEFINED_V_DATATYPE v_C, v_N1, v_N2, v_W1, v_W2, v_S1, v_S2, v_E1, v_E2, v_res;
        ...
        for (y = left_y_compute_t[DIMT]; y < right_y_load; y++)
        {
            ...
            if (DIMT >= 2)
            {
                for (global_x = left_x_load + R * (t + 1) - aligned, x = R * (t + 1) - aligned; global_x < right_x_load - R * (t + 1); global_x += 4, x += 4)
                {
                    ...
                    v_res = v_c0 * v_C + v_c1 * v_W1 + v_c2 * v_W2 + v_c3 * v_E1 + v_c4 * v_E2 + v_c5 * v_N1 + v_c6 * v_N2 + v_c7 * v_S1 + v_c8 * v_S2;
