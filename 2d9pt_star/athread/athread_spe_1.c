        ...
        for (g = 0; g < ng; g++)
        {
            ...
            for (y = left_y_compute_t[DIMT]; y < right_y_load; y++)
            {
                ...
                for (global_x = left_x_load + R * (t + 1), x = R * (t + 1); global_x < right_x_load - R * (t + 1); global_x++, x++)
                {
                    ...
                    S1 = S1_y_block_t * blockSize_x + x;
                    S2 = S2_y_block_t * blockSize_x + x;
                    local_tOut[x] = c0 * local_tIn_t[t][C] + c1 * local_tIn_t[t][W1] + c2 * local_tIn_t[t][W2] + c3 * local_tIn_t[t][E1] + c4 * local_tIn_t[t][E2] + c5 * local_tIn_t[t][N1] + c6 * local_tIn_t[t][N2] + c7 * local_tIn_t[t][S1] + c8 * local_tIn_t[t][S2];
                }
