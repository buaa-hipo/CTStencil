...
#pragma acc parallel loop copyin(ny, nx, t, y, x, c0, c1, c2, c3, c4, c5, c6, c7, c8) annotate(readonly(ny, nx, t, c0, c1, c2, c3, c4, c5, c6, c7, c8, global_In, global_Out))
        for (y = R * t; y < ny - R * t; y++)
        {
            ...
            for (x = R * t; x < nx - R * t; x++)
            {
                ...
                C4 = C0 + nx;
                C8 = C0 + nx + 1;
                global_Out[C0] = c0 * global_In[C0] + c1 * global_In[C1] + c2 * global_In[C2] + c3 * global_In[C3] + c4 * global_In[C4] + c5 * global_In[C5] + c6 * global_In[C6] + c7 * global_In[C7] + c8 * global_In[C8];
            }
        }
    }
    return out;
    ...
    for (t = 1; t <= nt; t++)
    {
        ...
        for (y = R * t; y < ny - R * t; y++)
        {
            for (x = R * t; x < nx - R * t; x++)
            {
                ...
                C4 = C0 + nx;
                C8 = C0 + nx + 1;
                temp[out][C0] = c0 * temp[in][C0] + c1 * temp[in][C1] + c2 * temp[in][C2] + c3 * temp[in][C3] + c4 * temp[in][C4] + c5 * temp[in][C5] + c6 * temp[in][C6] + c7 * temp[in][C7] + c8 * temp[in][C8];
            }
        }
    }
    return out;
