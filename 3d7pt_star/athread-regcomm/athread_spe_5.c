...
void spe_func(struct spe_parameter *spe_param)
{
    ...
    for (it = 0; it < NUMTIMESTEPS / DIMT; it++)
    {
        ...
        for (g = 0; g < ng; g++)
        {
            ...
            int256 load_info[4], load_info_get;
            ...
            int repeat_y = (right_y_load - left_y_load) / NTH;
            int left_y = (right_y_load - left_y_load) % NTH;
            for (z = left_z_compute_t[DIMT]; z < right_z_load; z++)
            {
                ...
                if (z + 1 < right_z_load)
                {
                    ...
                    for (y = left_y_load; y < right_y_load; y++)
                    {
                        ...
                        C_local = offset_z_block_0_preload * blockSize_y * blockSize_x + (y - left_y_load) * blockSize_x + 0;
                ...
                for (s = 0; s < comm_step; s++)
                {
                    ...
                    for (i = 0; i < repeat_y; i++)
                    {
                        ...
                        for (j = begin - aligned; j + 4 <= right_x_block_comm;)
                        {
                            ...
                            local_tIn_0[j++] = ((DEFINED_DATATYPE *)(&v_g))[1];
                            local_tIn_0[j++] = ((DEFINED_DATATYPE *)(&v_g))[2];
                            local_tIn_0[j++] = ((DEFINED_DATATYPE *)(&v_g))[3];
                        }
                        simd_load(v_p, &local_tIn_0[j]);
                        REG_PUTR(v_p, col_comm_CPE);
                        REG_GETR(v_g);
