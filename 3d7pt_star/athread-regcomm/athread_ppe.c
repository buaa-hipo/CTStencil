...
DEFINED_DATATYPE c[NUMPOINTS] = {-30. / 12., 16. / 12., 16. / 12., 16. / 12., 16. / 12., 16. / 12., 16. / 12.};
...
void fatal(char *s);
int computeTempCPE(DEFINED_DATATYPE *temp[2], int nt, int nz, int ny, int nx, int DIMT, int DIMX, int DIMY, int DIMZ, int NTH, int *slave_float_ops, int *slave_get_size, int *slave_put_size);
...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < NUMREPEATS; i++)
    {
        ...
        CPE_out = computeTempCPE(tempCPE, NUMTIMESTEPS, LAYERS, NUMROWS, NUMCOLS, DIMT, DIMX, DIMY, DIMZ, NTH, slave_float_ops, slave_get_size, slave_put_size);
        ...
        for (z = R * t; z < nz - R * t; z++)
        {
            for (y = R * t; y < ny - R * t; y++)
            {
                for (x = R * t; x < nx - R * t; x++)
                {
                    ...
                    B = C - nx * ny;
                    T = C + nx * ny;
                    temp[out][C] = c0 * temp[in][C] + c1 * temp[in][B] + c2 * temp[in][T] + c3 * temp[in][W] + c4 * temp[in][E] + c5 * temp[in][S] + c6 * temp[in][N];
                    // temp[out][C] = temp[in][C];
                }
            }
        }
    }
