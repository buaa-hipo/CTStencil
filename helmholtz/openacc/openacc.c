...
#pragma acc parallel loop copyin(nz, ny, nx, t, z, y, x, a, f, e, d) annotate(readonly(nz, ny, nx, t, a, f, e, d, global_In, global_Out))
        for (z = R * t; z < nz - R * t; z++)
        {
            for (y = R * t; y < ny - R * t; y++)
            {
                ...
                for (x = R * t; x < nx - R * t; x++)
                {
                    ...
                    T1 = C + nx * ny;
                    T2 = C + 2 * nx * ny;
                    global_Out[C] = (a - f) * global_In[C] + e * global_In[B1] + d * global_In[B2] + e * global_In[T1] + d * global_In[T2] + e * global_In[W1] + d * global_In[W2] + e * global_In[E1] + d * global_In[E2] + e * global_In[S1] + d * global_In[S2] + e * global_In[N1] + d * global_In[N2];
                }
            }
        }
    }
